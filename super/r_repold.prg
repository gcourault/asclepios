...
//-------------------------------------------------------------------------
FUNCTION OREPORTER(aInFields,aInTypes,aInLens)
local aTagged   := {}
local lUseQuery := .f.
local lUseTag   := .f.
...
IF !FILE(slsf_report()+".DBF")
     rMakeDbf()
ENDIF
if !SNET_USE(slsf_report(),"__REPORTS",.F.,5,.T.,"Error abriendo archivo REPORT. ¨Reintenta?")
   SELECT (nOldArea)
   READEXIT(lReadExit)
   setexact(lExact)
   setcolor(cOldColor)
...
DO WHILE !lDone
  ...
  @23,2 SAY "REPORT :"+IIF(EMPTY(aValues[M_TITLE]),"Nada ",upper(aValues[M_TITLE]))
  @23,55 SAY "USANDO  "+ALIAS()+".DBF"

  SET MESSAGE TO 22
  @3,2 PROMPT  'Traer REPORTE   '  MESSAGE "³ Traer un reporte del Disco                   "
  @4,2 PROMPT  'Crear REPORTE   '  MESSAGE "³ Comenzar un nuevo reporte                    "
  @5,2 PROMPT  'Grabar REPORTE  '  MESSAGE "³ Grabar un reporte al disco                   "
  @6,2 PROMPT  'Editar Reporte  '  MESSAGE "³ Definir las Columnas del Reporte             "
  @7,2 PROMPT  'Borrar Reporte  '  MESSAGE "³ Borrar definiciomes de Reporte               "
  @08,2 PROMPT 'Cabeza/Pie      '  MESSAGE "³ CABEZA y PIE del REPORTE                     "
  @09,2 PROMPT 'Filtrado        '  MESSAGE "³ Selecc¢n de registros                        "
  @10,2 PROMPT 'Agrupaciones    '  MESSAGE "³ Agrupaci¢n Primaria y Secundaria             "
  @11,2 PROMPT 'Otras Optiones  '  MESSAGE "³ Definiciones de P gina e impresi¢n           "
  @13,2 PROMPT 'Imprimir Reporte'  MESSAGE "³ Enviar a IMPRESORA o ARCHIVO                 "
  @15,2 PROMPT 'Estado          '  MESSAGE "³ Estado actual del reporte activo             "
  @16,2 PROMPT 'Salir           '  MESSAGE "³ Salir del Generador de Reportes              "
  MENU TO nSelection
  ...
  CASE nSelection == 2     // create report
    ...
    rTb:refreshall()
    rStabilize(rTb)
    @22,5 SAY "T¡tulo del REPORTE          " GET aValues[M_TITLE] pict "@K"
    READ
    SCROLL(22,1,23,78,0)
  ...
  CASE nSelection = 6 .AND. rLoaded()  && headers and footers
    @22,5 SAY "N£mero de l¡neas encab (1-9)   " GET aValues[M_NHEAD] PICT "9"
    @23,5 SAY "N£mero de l¡neas pie   (1-9)   " GET aValues[M_NFOOT] PICT "9"
    READ
  ...
  CASE nSelection = 7 .AND. rLoaded()    && record selection
    ...
    @15,28 PROMPT "Registros marcados"
    @16,28 PROMPT "Construir Consulta"
  ...
  CASE nSelection = 12  .or. lastkey()=K_ESC   // quitting time
    ...
    lDone := .t.
  otherwise
     Msg("No hay reporte definido. Cargue o cree uno")
  ENDCASE
  CLEAR TYPEAHEAD
ENDDO
aDbfFields := NIL
...
@12,12 SAY "Seleccione el orden : "
@13,12 PROMPT "Seleccionar un ¡ndice abierto"
@14,12 PROMPT "Ver el orden activo"     
@15,12 PROMPT "Desactivar el orden activo"
@17,12 PROMPT "Salir"
MENU TO nSelect
Unbox(cBox)
...
CASE nSelect = 1 .AND. lIndex  && select from open indexes
  ...
  @10,21 SAY "[Seleccione Indice a Usar]"
...
CASE nSelect = 2    && view selection
  Msg("La clasificaci¢n de la base es: ",IIF(EMPTY(INDEXKEY(0)),"Ninguna",left(indexkey(0),50) ),;
    "Grupo principal     :  ",IIF(EMPTY(aValues[M_MAJKEY]),"Ninguno",left(aValues[M_MAJKEY],50)),;
    "Grupo secundario    :  ",IIF(EMPTY(aValues[M_MINKEY]),"Ninguno",left(aValues[M_MINKEY],50)) )
...
WHILE .T.
  ...
  DO WHILE .T.
    ...
    LOCATE FOR TRIM(UPPER(aValues[M_TITLE]))==TRIM(UPPER(__reports->SF_TITLE)) ;
         .AND. !DELETED()
    IF FOUND()
      if messyn("El reporte ya existe","No regraba","Regraba")
        SCROLL(22,1,23,78,0)
        LOOP
      endif
    ELSE
...
IF EOF()
  Msg("No hay reportes de: "+cDbfName)
ENDIF
...
DO WHILE !EOF()
  ...
  if !empty(cStoredKey)
    ...
    if nFoundKey > 0
        ...
        msg("Usando Indice   =>"+aValues[M_NDXKEY])
    else
        aValues[M_NDXKEY] := ""
        aValues[M_MAJKEY] := ""
        aValues[M_MINKEY] := ""
...
@11,8  SAY "Se pueden agrupar los datos en su reporte."
@12,8  SAY "Un CAMBIO DE GRUPO es donde cambian una"
@13,8  SAY "de las claves de los INDICES. El SUBTOTAL"
@14,8  SAY "se imprime cuando hay un CAMBIO DE GRUPO."
@15,8  SAY "Este generador de informes soporta el GRUPO MAYOR y el "
@16,8  SAY "SECUNDARIO. Ahora se preguntar  por los GRUPOS"
@18,8 SAY "[Pulse una tecla ....]"
INKEY(0)
Unbox(cBox)

IF Messyn("¨Selecciona el grupo mayor desde el ¡ndice?")
  nChoice := Mchoice(aMajorKeys,10,10,20,60,"Seleccione grupo MAYOR")
  IF nChoice > 0
    aValues[M_MAJKEY] := aMajorKeys[nChoice]
  ENDIF

  IF nSegments > 1 .AND. nChoice > 0 .AND. nChoice < nSegments
    IF Messyn("¨Selecciona el grupo secundario desde el ¡ndice?")
      for nCounter = 1 TO nChoice
        Adel(aMinorKeys,1)
      NEXT
      asize(aMinorKeys,len(aMinorKeys)-nChoice)
      nChoice := Mchoice(aMinorKeys,10,10,20,60,"Seleccione grupo MENOR")
      IF nChoice > 0
...
Setcolor(sls_normcol())
CLEAR
@ 0,0 SAY  " ÄÄ Generador de REPORTES 1.0 ÄÄ"
dispbox(1,0,24,79)
@ 21,0 SAY "ÃÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ´"
@ 2,19 to 20,19
RETURN ''

//-------------------------------------------------------------------------
static FUNCTION rStabilize(rTb)
while !rTb:stabilize()
end
RETURN ''
...
local rTb
nElement := 1
rTb := tbrowseNew(2,20,20,78)
rTb:addcolumn(tbcolumnnew("#",{||trans(nElement,"99")}   )  )
rTb:addcolumn(tbcolumnnew("Contenido Columna",{||padr(aColumns[nElement],17)}   )  )
rTb:addcolumn(tbcolumnnew("T¡tulo Columna",{||padr(aTitles[nElement],19)}   )  )
rTb:addcolumn(tbcolumnnew("Ancho",{||padc(aWidths[nElement],5)}  ))
rTb:addcolumn(tbcolumnnew("Format" ,{||padc(aPictures[nElement],5)}  ))
...
@nTop,4 SAY "[Encabezamientos y Pies]"
...
//-------------------------------------------------------------------------
static FUNCTION rBrowseEdit(rTb)
local nLastkey

Scroll(2,1,20,17,0)
@ 2,2 SAY "Teclas de Edici¢n"
@ 3,1 SAY "ÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ"
@ 5,4 SAY "<"+CHR(24)+" "+CHR(27)+" "+CHR(25)+" "+CHR(26)+">"
@ 6,4 SAY "para moverse"
@ 8,4 SAY "<ENTER>"
@ 9,4 SAY "para cambiar"
@ 11,4 SAY "<INSERT>"
@ 12,4 SAY "para agregar"
@ 14,4 SAY "<DELETE>"
@ 15,4 SAY "para borrar"
@ 17,4 SAY "<ESCAPE>"
@ 18,4 SAY "para salir"
SCROLL(22,1,23,78,0)
...
DO WHILE .T.
  ...
  if aValues[M_NCOLS] > 0
    ...
    CASE rTb:colpos=4
      @ 23,6 SAY "Ancho Column: "+aWidths[nElement]
...
CASE nWhichColumn = 3   && title
  ...
  @22,5 SAY "Ingrese nuevo ancho " GET cTitleLen PICT "99"
  READ
  SCROLL(22,1,23,78,0)
