...
// No me gustan las variables p˙blicas, eliminada.
// memvar aReport

// ModificaciÛn provisional
// Lo correcto es sustituir CRLF por EOL (end of line) en todo el archivo
// pero hay algunas funciones que dependen de ello (pdfTextNextPara())
#define EOL chr(10)
// #undef  CRLF 
// #define CRLF EOL
...
// Fin de funciones aÒadidas.


                                                                              /*
‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹                                    */
function pdfOpen( cFile, nLen, lOptimize, cfontFile )                                    /*
‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹                                    */
local cTemp, nI, nJ, n1, n2 := 896, n12
DEFAULT nLen to 200
DEFAULT lOptimize to .f.
default cfontFile to "fonts.dat"
   ...
   cTemp := memoread(cfontFile) // times, times-bold, times-italic, times-bolditalic, helvetica..., courier... // 0.04
   n1 := len( cTemp ) / ( 2 * n2 )
...
// srg:mod 10.11.05
// padl( numero, 10, "0" ) con algunos d· similar a : 254808e+00 
      // cTemp += padl( aReport[ REFS ][ nI ], 10, "0") + " 00000 n" + CRLF
      cTemp += strzero(aReport[ REFS ][ nI ], 10) + " 00000 n" + padl(CRLF,2)
// srg:end
   next
...
// srg, 22.11.05, modificado.
// Nota: La linea eliminada permite que 'xpdf' muestre bien las im·genes
   cTemp := ;
...
* debuglog( "PAGEFONTS=", aReport[PAGEFONTS])
...
//   pdfAtSay( cString, pdfR2M( nRow ), IIF( cUnits == "R", aReport[ PDFLEFT ] + ( aReport[ PAGEX ] / 72 * 25.4 - 2 * aReport[ PDFLEFT ] ) * nCol / aReport[ REPORTWIDTH ] - nAdj, nCol ) - nLen, "M", lExact )
   pdfAtSay( cString, iif( cUnits=='R',pdfR2M( nRow ),nRow), IIF( cUnits == "R", aReport[ PDFLEFT ] + ( aReport[ PAGEX ] / 72 * 25.4 - 2 * aReport[ PDFLEFT ] ) * nCol / aReport[ REPORTWIDTH ] - nAdj, nCol ) - nLen, "M", lExact )
return nil
                                                                              /*
‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹‹                            */
...
#ifdef __CLIP__
   SWPRUNCMD( "cp temp.tmp " + cFile + " > /dev/null", 0, "", "")
#else
...
local ret, out:=space(0), err:=space(0)

ret := syscmd( cCommand, @out, @err )
outlog( "syscmd.ret=", ret )
outlog( "syscmd.out=", out )
outlog( "syscmd.err=", err )
