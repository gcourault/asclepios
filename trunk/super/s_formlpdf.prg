  ...
  *- do action based on request
  ...
  CASE nMainSelect = 5 .AND. !EMPTY(cCurrFormDes)
    ...
    if cShowFilter == "UN SOLO REGITRO"
      sffl_printpdf(cDestination,bLocater,.t.)
    else
      sffl_printpdf(cDestination,bLocater,.f.)
    endif
  CASE nMainSelect = 6
...
static FUNCTION sffl_printpdf(cDestination,bLocater,lSingle)
local cBox1,cBox2
